...
namespace Farming_Simulator_Mod_Manager {
    ...
    public partial class EditMod : Form {
        ...
        private void bLoadGameFiles_Click(object sender, EventArgs e) {
            ...
            switch (gam) {
                case "FS11":
                    pth = reg.Read(Fs11RegKeys.FS11_DATA_DIR);
                    if (pth.IsNullOrEmpty()) {
                        ...
                        reg.Write(Fs11RegKeys.FS11_DATA_DIR, ofd.SelectedPath + "\\vehicles\\");
                ...
                case "FS13":
                    pth = reg.Read(Fs13RegKeys.FS13_DATA_DIR);
                    if (pth.IsNullOrEmpty()) {
                        tree.Nodes.Clear();
                        var ofd = new FolderBrowserDialog {Description = @"Navigate to your Game Data Folder"};
                        ofd.ShowDialog();
                        if (!ofd.SelectedPath.FolderExists()) return;
                        reg.Write(Fs13RegKeys.FS13_DATA_DIR, ofd.SelectedPath + "\\vehicles\\");
                        LoadTree(ofd.SelectedPath);
                        return;
                    }
                ...
                case "FS19":
                    pth = reg.Read(FS19RegKeys.FS19_DATA_DIR);
                    if (pth.IsNullOrEmpty()) {
                        tree.Nodes.Clear();
                        var ofd = new FolderBrowserDialog {Description = @"Navigate to your Game Data Folder"};
                        ofd.ShowDialog();
                        if (!ofd.SelectedPath.FolderExists()) return;
                        reg.Write(FS19RegKeys.FS19_DATA_DIR, ofd.SelectedPath + "\\vehicles\\");
                        LoadTree(ofd.SelectedPath);
                        return;
                    }
        ...
        private void bLoadSaveGames_Click(object sender, EventArgs e) {
            ...
            switch (gam) {
                case "FS11":
                    savePath = reg.Read(Fs11RegKeys.FS11_SAVE_GAMES);
                    if (savePath.IsNullOrEmpty()) {
                        var ofd = new FolderBrowserDialog {
                            Description = @"Navigate to the FS 11 SaveGame folder",
                            ShowNewFolderButton = false
                        };
                        ofd.ShowDialog();
                        if (!ofd.SelectedPath.IsNullOrEmpty()) {
                            savePath = ofd.SelectedPath;
                            reg.Write(Fs11RegKeys.FS11_SAVE_GAMES, savePath);
                        }
                        else {
                            return;
                        }
                ...
                case "FS13":
                    savePath = reg.Read(Fs13RegKeys.FS13_SAVE_GAMES);
                    if (savePath.IsNullOrEmpty()) {
                        var ofd = new FolderBrowserDialog {
                            Description = @"Navigate to the FS 13 SaveGame folder",
                            ShowNewFolderButton = false
                        };
                        ofd.ShowDialog();
                        if (!ofd.SelectedPath.IsNullOrEmpty()) {
                            savePath = ofd.SelectedPath;
                            reg.Write(Fs13RegKeys.FS13_SAVE_GAMES, savePath);
                        }
                        else {
                            return;
                        }
                ...
                case "FS19":
                    savePath = reg.Read(FS19RegKeys.FS19_SAVE_GAMES);
                    if (savePath.IsNullOrEmpty()) {
                        var ofd = new FolderBrowserDialog {
                            Description = @"Navigate to the FS 19 SaveGame folder",
                            ShowNewFolderButton = false
                        };
                        ofd.ShowDialog();
                        if (!ofd.SelectedPath.IsNullOrEmpty()) {
                            savePath = ofd.SelectedPath;
                            reg.Write(FS19RegKeys.FS19_SAVE_GAMES, savePath);
                        }
                        else {
                            return;
                        }
        ...
        private void bSaveWork_Click(object sender, EventArgs e) {
            ...
            switch (gam) {
                case "FS11":
                    FolderCreator.CreatePublicFolders(reg.Read(Fs11RegKeys.FS11_WORK));
                    break;
                
                case "FS13":
                    FolderCreator.CreatePublicFolders(reg.Read(Fs13RegKeys.FS13_WORK));
                    break;
                ...
                case "FS19":
                    FolderCreator.CreatePublicFolders(reg.Read(FS19RegKeys.FS19_WORK));
                    break;
            }
        }
